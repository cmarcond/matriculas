"""
buscar_programas.py - Busca o programa/curso de cada aluno na aba Professor

Navega ao "Histórico interno" de cada aluno, aceita o alert de confirmação,
extrai o nome do programa (linha antes de ÁREA/CURRÍCULO) e salva em CSV.

Uso:
  python buscar_programas.py [--headless/--no-headless] [--limit N] [--output FILE]
"""
...
WAIT = 2
MAX_RETRIES = 3
DEFAULT_OUTPUT = "programas_professor.csv"
...
def load_existing(csv_path):
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            existing[row["codigo"]] = row["programa"]
    return existing


def append_to_csv(csv_path, turma, codigo, nome, programa):
    write_header = not csv_path.exists() or csv_path.stat().st_size == 0
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["turma", "codigo", "nome", "programa"])
        writer.writerow([turma, codigo, nome, programa])
...
def fetch_programa(page, student):
    """Navigate to student's Histórico interno, extract program name. Returns str or ''."""
    ...
    # Poll the RelatorioFrame until ÁREA/CURRÍCULO is found
    programa = ""
    for _ in range(10):
        ...
        if frame:
            try:
                ...
                if "ÁREA/CURRÍCULO:" in body_text:
                    # Program name is the line right before ÁREA/CURRÍCULO:
                    match = re.search(r"(.+)\n\s*ÁREA/CURRÍCULO:", body_text)
                    if match:
                        programa = match.group(1).strip()
                    break
            except Exception:
                pass
        # Fallback: try main page body
        if not programa:
            try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Busca programa/curso dos alunos na aba Professor"
    )
    parser.add_argument(
        "--headless",
    ...
    log("=" * 60)
    log("Busca de Programas — Aba Professor")
    log("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if existing:
                log(f"Programas já coletados (CSV existente): {len(existing)}", "SKIP")
            ...
            if not pending:
                log("Todos os programas já foram coletados!", "OK")
                browser.close()
                return
            ...
            for i, student in enumerate(pending, 1):
                ...
                for attempt in range(MAX_RETRIES):
                    try:
                        ...
                        programa = fetch_programa(page, fresh)
                        if programa:
                            break
                        else:
                            log(f"  Programa não encontrado (tentativa {attempt + 1}/{MAX_RETRIES})", "WARN")
                    except Exception as e:
                        log(f"  Erro (tentativa {attempt + 1}/{MAX_RETRIES}): {e}", "ERROR")
                        time.sleep(2)

                if programa:
                    append_to_csv(csv_path, student.turma, student.codigo, student.nome, programa)
                    log(f"[{i}/{total}] [{student.codigo}] {student.nome} — {programa}", "OK")
                    success += 1
                else:
                    append_to_csv(csv_path, student.turma, student.codigo, student.nome, "")
                    log(f"[{i}/{total}] [{student.codigo}] {student.nome} — programa não encontrado", "ERROR")
                    fail += 1
            ...
            print()
            log("=" * 60)
            log(f"Resumo: {success} programa(s) encontrado(s), {fail} falha(s), {already} já coletado(s)")
