"""
buscar_historico.py - Busca email e programa de cada aluno na aba Professor

Navega ao "Histórico interno" de cada aluno, aceita o alert de confirmação,
extrai E-MAIL e programa (curso) em uma única visita e salva em CSV.

Uso:
  python buscar_historico.py [--headless/--no-headless] [--limit N] [--output FILE]
"""
...
WAIT = 2
MAX_RETRIES = 3
DEFAULT_OUTPUT = "historico_professor.csv"
...
def load_existing(csv_path):
    """Load already-fetched records from CSV. Returns set of codigos."""
    existing = set()
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            existing.add(row["codigo"])
    return existing


def append_to_csv(csv_path, turma, codigo, nome, email, programa):
    write_header = not csv_path.exists() or csv_path.stat().st_size == 0
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["turma", "codigo", "nome", "email", "programa"])
        writer.writerow([turma, codigo, nome, email, programa])


# ---------------------------------------------------------------------------
# Data extraction (email + programa in one visit)
# ---------------------------------------------------------------------------

def fetch_student_data(page, student):
    """Navigate to Histórico interno, extract email and programa. Returns (email, programa)."""
    ...
    # Dismiss the "Informação" modal if present
    try:
    ...
    # Poll the RelatorioFrame until both fields are found
    email = ""
    programa = ""
    for _ in range(10):
        ...
        if frame:
            try:
                ...
                if not email:
                    m = re.search(r"E-MAIL:\s*(\S+@\S+)", body_text, re.IGNORECASE)
                    if m:
                        email = m.group(1).strip().rstrip(".,:;")

                if not programa:
                    m = re.search(r"(.+)\n\s*ÁREA/CURRÍCULO:", body_text)
                    if m:
                        programa = m.group(1).strip()

                if email and programa:
                    break
            except Exception:
                pass

        # Fallback: main page body
        try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Busca email e programa dos alunos na aba Professor"
    )
    parser.add_argument(
        "--headless",
    ...
    log("=" * 60)
    log("Busca de Histórico — Aba Professor")
    log("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if existing:
                log(f"Já coletados (CSV existente): {len(existing)}", "SKIP")
            ...
            if not pending:
                log("Todos os dados já foram coletados!", "OK")
                browser.close()
                return
            ...
            for i, student in enumerate(pending, 1):
                ...
                for attempt in range(MAX_RETRIES):
                    try:
                        ...
                        email, programa = fetch_student_data(page, fresh)
                        if email and programa:
                            break
                        else:
                            missing = []
                            ...
                            log(f"  {', '.join(missing)} não encontrado(s) (tentativa {attempt + 1}/{MAX_RETRIES})", "WARN")
                    except Exception as e:
                        log(f"  Erro (tentativa {attempt + 1}/{MAX_RETRIES}): {e}", "ERROR")
                        time.sleep(2)

                append_to_csv(csv_path, student.turma, student.codigo, student.nome, email, programa)
                if email and programa:
                    log(f"[{i}/{total}] [{student.codigo}] {student.nome} — {email} | {programa}", "OK")
                    success += 1
                else:
                    log(f"[{i}/{total}] [{student.codigo}] {student.nome} — dados incompletos (email={email or '?'}, programa={programa or '?'})", "ERROR")
                    fail += 1
            ...
            print()
            log("=" * 60)
            log(f"Resumo: {success} completo(s), {fail} incompleto(s), {already} já coletado(s)")
